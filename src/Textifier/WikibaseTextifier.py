"""Data structures for Wikidata/Wikibase entities and serialization helpers."""
...
class WikibaseText:
    """Object for Wikibase plain text values."""
...
class WikibaseMonolingualText:
    """Object for Wikibase monolingual text values."""
...
@dataclass(slots=True)
class WikibaseCoordinates:
    """Object for Wikibase coordinate values."""
...
@dataclass(slots=True)
class WikibaseTime:
    """Object for Wikibase time values."""
...
@dataclass(slots=True)
class WikibaseQuantity:
    """Object for Wikibase quantity values."""
...
class WikibaseEntity:
    """Object for Wikibase entities."""

    id: str
    label: Optional[Any] = None
    description: Optional[str] = None
    aliases: List[str] = field(default_factory=list)
    claims: List["WikibaseClaim"] = field(default_factory=list)

    def __bool__(self) -> bool:
        """Return whether this entity has a usable id and label."""
        id_check = bool(self.id)
        label_check = self.label is not None and str(self.label) != ""
        return id_check and label_check
...
@dataclass(slots=True)
class WikibaseClaim:
    """Object for Wikibase claims."""

    subject: WikibaseEntity
    property: WikibaseEntity
    values: List["WikibaseClaimValue"] = field(default_factory=list)
    datatype: str = "string"
...
@dataclass(slots=True)
class WikibaseClaimValue:
    """Object for Wikibase claim values."""

    claim: WikibaseClaim
    value: Optional[
        Union[
            WikibaseEntity, WikibaseQuantity, WikibaseTime, WikibaseCoordinates, WikibaseText, WikibaseMonolingualText
        ]
    ] = None
    qualifiers: List[WikibaseClaim] = field(default_factory=list)
    references: List[List[WikibaseClaim]] = field(default_factory=list)
    ...
    def to_text(self, lang="en") -> str:
        ...
        if isinstance(self.value, WikibaseEntity):
    ...
    def to_json(self) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(self.value, WikibaseEntity) and isinstance(value_json, dict):
