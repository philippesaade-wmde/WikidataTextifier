...
from .utils import get_wikibase_json_by_ids
...
class WikibaseLabel(Base):
    ...
    def initialize_database():
        ...
        try:
            ...
            WikibaseLabel._migrate_labels_table_for_wikibase()
            return True
    ...
    def add_bulk_labels(data, wb_url: str = DEFAULT_WIKIBASE_URL):
        ...
        normalized_wb_url = WikibaseLabel._normalize_wb_url(wb_url)
        rows = []
        for row in data:
            normalized_row = {
                "id": row["id"],
                "wikibase_url": WikibaseLabel._normalize_wb_url(row.get("wikibase_url", normalized_wb_url)),
    ...
    def add_label(id, labels, wb_url: str = DEFAULT_WIKIBASE_URL):
        ...
        with Session() as session:
            try:
                new_entry = WikibaseLabel(
                    id=id,
                    wikibase_url=WikibaseLabel._normalize_wb_url(wb_url),
                    labels=labels,
                )
    ...
    def get_labels(id, wb_url: str = DEFAULT_WIKIBASE_URL):
        ...
        try:
            with Session() as session:
                ...
                item = (
                    session.query(WikibaseLabel)
                    .filter(
                        WikibaseLabel.id == id,
                        WikibaseLabel.wikibase_url == normalized_wb_url,
                        WikibaseLabel.date_added >= date_limit,
                    )
                    .first()
                )
        ...
        labels = WikibaseLabel._get_labels_wdapi(id, wb_url=normalized_wb_url).get(id)
        if labels:
            WikibaseLabel.add_label(id, labels, wb_url=normalized_wb_url)
    ...
    def get_bulk_labels(ids, wb_url: str = DEFAULT_WIKIBASE_URL):
        ...
        try:
            with Session() as session:
                ...
                date_limit = datetime.now() - timedelta(days=LABEL_TTL_DAYS)
                rows = (
                    session.query(WikibaseLabel.id, WikibaseLabel.labels)
                    .filter(
                        WikibaseLabel.id.in_(ids),
                        WikibaseLabel.wikibase_url == normalized_wb_url,
                        WikibaseLabel.date_added >= date_limit,
                    )
                    .all()
        ...
        if missing_ids:
            missing_labels = WikibaseLabel._get_labels_wdapi(missing_ids, wb_url=normalized_wb_url)
            labels.update(missing_labels)

            # Cache labels
            WikibaseLabel.add_bulk_labels(
    ...
    def _get_labels_wdapi(ids, wb_url: str = DEFAULT_WIKIBASE_URL):
        ...
        """
        entities_data = get_wikibase_json_by_ids(ids, action_api_url=wb_url, props="labels")
        entities_data = WikibaseLabel._compress_labels(entities_data)
    ...
    def get_all_missing_labels_ids(data):
        ...
        if isinstance(data, dict):
            ...
            for _, value in data.items():
                ids_list = ids_list | WikibaseLabel.get_all_missing_labels_ids(value)

        elif isinstance(data, list):
            for item in data:
                ids_list = ids_list | WikibaseLabel.get_all_missing_labels_ids(item)
...
class LazyLabelFactory:
    ...
    def __init__(self, lang="en", fallback_lang="en", wb_url: str = DEFAULT_WIKIBASE_URL):
        ...
        self.wb_url = WikibaseLabel._normalize_wb_url(wb_url)
    ...
    def resolve_all(self):
        ...
        label_data = WikibaseLabel.get_bulk_labels(list(self._pending_ids), wb_url=self.wb_url)
    ...
    def get_label(self, qid: str) -> str:
        ...
        label = WikibaseLabel.get_lang_val(label_dict, lang=self.lang, fallback_lang=self.fallback_lang)
        return label
